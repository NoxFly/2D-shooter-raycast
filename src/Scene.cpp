...
void Scene::updatePlayer() {
	...
	// detect physical collision with walls
	m_player.collide(m_walls);
}
...
void Scene::createWall(int x, int y, int width, int height) {
	...
	m_walls.push_back(
		new Wall(
			(x-1)  * m_cellContext.ratio,
			(y-1)  * m_cellContext.ratio,
			width  * m_cellContext.ratio,
			height * m_cellContext.ratio
		)
	);
}